...
# Set multi-threading for faster processing on CPU
torch.set_num_threads(10)  # Adjust this based on the number of CPU cores you have
...
def inference(net, img_path='', output_path='./', output_name='f', use_gpu=False):
    ...
    # Move to GPU only if use_gpu is True
    if use_gpu:
        adj2_test = adj2_test.cuda()
        adj3_test = adj3_test.cuda()
        adj1_test = adj1_test.cuda()
...
if __name__ == '__main__':
    ...
    if not opts.loadmodel == '':
        x = torch.load(opts.loadmodel, map_location=torch.device('cpu'),weights_only=True)
